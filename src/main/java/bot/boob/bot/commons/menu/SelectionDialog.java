 ...
 */
package bot.boob.bot.commons.menu;
...
/**
 * A {@link Menu Menu} implementation that creates a listed display of text choices horizontally
 * that users can scroll through using reactions and make selections.
 *
 * @author John Grosh
 */
public class SelectionDialog extends Menu {
    ...
    SelectionDialog(
            ...
            String defaultLeft,
            String defaultRight,
            Function<Integer, Color> color,
            boolean loop,
            BiConsumer<Message, Integer> success,
    ...
    /**
     * Shows the SelectionDialog as a new {@link Message Message} in the provided {@link
     * MessageChannel MessageChannel}, starting with the first selection.
     *
    ...
    /**
     * Displays this SelectionDialog by editing the provided {@link Message Message}, starting with
     * the first selection.
     *
    ...
    /**
     * Shows the SelectionDialog as a new {@link Message Message} in the provided {@link
     * MessageChannel MessageChannel}, starting with the number selection provided.
     *
     * @param channel   The MessageChannel to send the new Message to
    ...
    public void showDialog(MessageChannel channel, int selection) {
        ...
        else if (selection > choices.size()) selection = choices.size();
        ...
        action.queue(
                m -> {
                    if (choices.size() > 1) {
                        ...
                        m.addReaction(DOWN)
                                .queue(v -> selectionDialog(m, selection), v -> selectionDialog(m, selection));
                    } else {
                        m.addReaction(SELECT).queue();
                        m.addReaction(CANCEL)
                                .queue(v -> selectionDialog(m, selection), v -> selectionDialog(m, selection));
                    }
                });
    }
        ...
        waiter.waitForEvent(
                ...
                event -> {
                    ...
                    if (!(UP.equals(event.getReaction().getReactionEmote().getName())
                            ...
                            || SELECT.equals(event.getReaction().getReactionEmote().getName()))) return false;
                ...
                event -> {
                    ...
                    switch (event.getReaction().getReactionEmote().getName()) {
                        case UP:
                            if (newSelection > 1) newSelection--;
                            else if (loop) newSelection = choices.size();
                            break;
                        case DOWN:
                            if (newSelection < choices.size()) newSelection++;
                            else if (loop) newSelection = 1;
                            break;
                        case SELECT:
    ...
    private Message render(int selection) {
        ...
        if (content != null) mbuilder.append(content);
        return mbuilder
                .setEmbed(
                        new EmbedBuilder()
    ...
    /**
     * The {@link Menu.Builder Menu.Builder} for a {@link SelectionDialog SelectuibDialog}.
     *
     * @author John Grosh
     */
    public static class Builder extends Menu.Builder<Builder, SelectionDialog> {
        ...
        /**
         * Builds the {@link SelectionDialog SelectionDialog} with this Builder.
         *
         * @return The OrderedMenu built from this Builder.
         * @throws IllegalArgumentException If one of the following is violated:
         *                                  <ul>
         *                                  <li>No {@link EventWaiter EventWaiter} was set.
         *                                  <li>No choices were set.
         *                                  <li>No action {@link Consumer Consumer} was set.
         *                                  </ul>
         */
        @Override
         ...
         * for the color of the embed to change depending on the selection number.
         *
         * @param color A Function that uses current selection number to get a Color for the
         *              MessageEmbed
         * @return This builder
         */
        ...
        /**
         * Sets the text of the {@link Message Message} to be displayed when the {@link SelectionDialog
         * SelectionDialog} is built.
         *
         * <p>This is displayed directly above the embed.
         *
         * @param text The Message content to be displayed above the embed when the SelectionDialog is
         *             built
         * @return This builder
         */
        ...
        /**
         * Sets the text of the {@link Message Message} to be displayed relative to the current
         * selection number as determined by the provided {@link Function Function}. <br>
         * As the selection changes, the Function will re-process the current selection number, allowing
         * for the displayed text of the Message to change depending on the selection number.
         *
        ...
        /**
         * Sets the text to use on either side of all unselected items. This will not be applied to the
         * selected item. <br>
         ...
         *
         * @param loop {@code true} if pressing up while at the top selection should loop to the bottom,
        ...
        /**
         * Sets a {@link BiConsumer BiConsumer} action to perform once a selection is made. <br>
         * The {@link Message Message} provided is the one used to display the menu and the {@link
         * Integer Integer} is that of the selection made by the user, and selections are in order of
         * addition, 1 being the first String choice.
         *
        ...
        /**
         * Sets a {@link Consumer Consumer} action to take if the menu is cancelled, either via the
         * cancel button being used, or if the SelectionDialog times out.
         *
