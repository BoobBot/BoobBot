...
class Spank : AsyncSlashCommand {
    override suspend fun executeAsync(ctx: SlashContext) {
        ...
        val res = BoobBot.requestUtil.get("https://boob.bot/api/v2/img/spank", headersOf("Key", BoobBot.config.BB_API_KEY)).await()?.json()
            ?: return ctx.reply(Formats.error(" oh? something broken af"))

        ctx.reply {
            setTitle("<:spank:866431559557054464> ${ctx.member.effectiveName} Spanks ${target.effectiveName}")
