...
class Leaderboard : AsyncCommand {
    ...
    suspend fun cash(ctx: Context) {
        var msg = ""
        var count = 0
        BoobBot.database.getAllUsers().find().sort(BasicDBObject("balance", -1)).limit(15).iterator().forEach { u ->
            if (count >= 15) {
                return@forEach
            }
            try {
                ...
                msg += "$count: ***${user.name}***  balance: $***${u["balance"]}***\n"
                count++
            } catch (e: Exception) {
                BoobBot.database.getAllUsers().deleteOne(Filters.eq("_id", u["_id"]))
                return@forEach
    ...
    suspend fun level(ctx: Context) {
        var msg = ""
        var count = 0
        BoobBot.database.getAllUsers().find().sort(BasicDBObject("level", -1)).limit(20).iterator().forEach { u ->
            if (count >= 15) {
                return@forEach
            }
            try {
                ...
                msg += "$count: ***${user.name}***  level: $***${u["level"]}***\n"
                count++
            } catch (e: Exception) {
                BoobBot.database.getAllUsers().deleteOne(Filters.eq("_id", u["_id"]))
                return@forEach
    ...
    suspend fun rep(ctx: Context) {
        var msg = ""
        var count = 0
        BoobBot.database.getAllUsers().find().sort(BasicDBObject("rep", -1)).limit(15).iterator().forEach { u ->
            if (count >= 15) {
                return@forEach
            }
            try {
