...
class Suck : AsyncCommand {
    ...
    override suspend fun executeAsync(ctx: Context) {
        ...
        val res =
            BoobBot.requestUtil.get("https://boob.bot/api/v2/img/suck", Headers.of("Key", BoobBot.config.BB_API_KEY))
                .await()?.json()
                ?: return ctx.send(
        ...
        ctx.embed {
            setTitle("<a:nekosuck:501483984136699904> ${ctx.author.name} Sucks off ${target.name}")
