...
class dom : AsyncCommand {
    ...
    override suspend fun executeAsync(ctx: Context) {
        ...
        val res =
            BoobBot.requestUtil.get("https://boob.bot/api/v2/img/dom", Headers.of("Key", BoobBot.config.BB_API_KEY))
                .await()?.json()
                ?: return ctx.send(
        ...
        ctx.embed {
            setTitle("<:dom:866457723788329020> ${ctx.author.name} Dominates ${target.name}")
