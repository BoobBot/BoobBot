...
class lick : AsyncCommand {
    ...
    override suspend fun executeAsync(ctx: Context) {
        ...
        val res =
            BoobBot.requestUtil.get("https://boob.bot/api/v2/img/lick", Headers.of("Key", BoobBot.config.BB_API_KEY))
                .await()?.json()
                ?: return ctx.send(
        ...
        ctx.embed {
            setTitle("<a:play:866441014830563388> ${ctx.author.name} licks ${target.name}")
